...
from swarm_control_package.srv import NewTarget, SpecialCommands
import math
...
class Arena( object ):
    ...
    drones_assignment_dict =  {                               # allotted targets
        #'Allegiance' : ['1','2','20', '21','22', '24', '11', '12', '13', '14', '15' , '3', '4', '5', '6', '7', '8', '9', '10', '16' , '17', '18', '19', '23', '0'],
        'Allegiance' : ['1','2','3', '4'],
        'Royal_Guard': ['1','2','20', '21','22', '24', '11', '12', '13', '14', '15' , '3', '4', '5'],
        'Silky'      : [ '3', '5']
    ...
    def fiducial_callback(self, msg):
        ...
        for key in self.drones_targets_dict.keys():
            temp_dict = temp_dict + list(set(self.drones_targets_dict[key] ) )
        ...
        for transform in msg.transforms:
            ...
            if not fiducial_id in temp_dict:
                ...
                time = rospy.Time.now().to_sec() - self.last_Sighting # - rospy.Time.now().to_sec()
                self.last_Sighting = rospy.Time.now().to_sec()
                self.captures += 1
    ...
    def validate_and_assign(self, fiducial_id) :
        ...
        for key in self.drones_targets_dict.keys(): # to iterate which drone can take up, and which qualifies to take uo w.r.t. the current traget list.
            if fiducial_id in self.drones_assignment_dict[key]: # does this drone qualify
                if len(self.drones_targets_dict[key]) == 0: # automatic qualification. to level off assignments to each.
                    k = key
                    break
                for i in range(len(self.drones_targets_dict[key])): # else, calculates which waypoint is closest to the newly spotted flag.
        ...
        try:
            ...
            print('Updated dictionary for ', k, ' is ', self.drones_targets_dict[k])
    ...
    def update_target_list_after_neutralization_Common(self, req):
        ...
        for key in self.drones_targets_dict.keys():
            self.drones_targets_dict[key] = list(set(self.drones_targets_dict[key])) # to remove duplicates
    ...
    def main(self):
        ...
        rospy.Subscriber("flags/fiducial_transforms", FiducialTransformArray, self.fiducial_callback, queue_size=6)
        ...
        while not rospy.is_shutdown():
            ...
            print(' Allegiance is ', self.drones_targets_dict['Allegiance'])
            print('Royal_Guard is ', self.drones_targets_dict['Royal_Guard'])
            print('      Silky is ', self.drones_targets_dict['Silky'], '\n')
            #temp_dict = []
            '''print('LAst sighting ', self.last_Sighting)
            print('Median' , self.median)
            print('Time now ', rospy.Time.now().to_sec())#)'''
            if not self.start_of_game:
                if ( (rospy.Time.now().to_sec() - self.last_Sighting ) < self.median * 2 )and len(self.drones_targets_dict['Allegiance']) == 0:
                    ...
                    print('S P E ')
                    pub.publish(msg)
                    #self.special_command_service_client_Allegiance("Special")
                    #self.special_command_service_client_Royal_Guard("Special")
                    #self.special_command_service_client_Silky("Special")
                    #rospy.signal_shutdown()                #elif self.maneuver:
                    #self.special_command_service_client_Allegiance("Abort")
                    #self.special_command_service_client_Royal_Guard("Abort")
                    #self.special_command_service_client_Silky("Abort")
                    #self.maneuver = False
                temp_dict = []
                rate.sleep()
